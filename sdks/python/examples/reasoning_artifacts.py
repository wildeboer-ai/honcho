...
hypotheses = client.get_hypotheses(
    observer="user_123", observed="user_123", status="active"
)
...
if hypotheses:
    ...
    predictions = client.get_hypothesis_predictions(hypothesis_id, status="unfalsified")
    ...
    if genealogy["reasoning_metadata"]:
        print(
            f"Evolution reason: {genealogy['reasoning_metadata'].get('reason', 'N/A')}"
        )
...
for pred in similar[:3]:  # Show top 3
    print(f"  - {pred['content']} (status: {pred['status']})")
...
if traces:
    ...
    for query in trace.get("search_queries") or []:
        print(f"  - {query}")
    if trace.get("contradicting_premise_ids"):
        print(
            f"Contradicting Premises Found: {len(trace['contradicting_premise_ids'])}"
        )
...
inductions = client.get_inductions(
    observer="user_123", observed="user_123", confidence="high"
)
...
for pattern_type in pattern_types:
    inductions = client.get_inductions(
        observer="user_123", observed="user_123", pattern_type=pattern_type
    )
    if inductions:
...
print(f"Found {len(peer_hypotheses)} active hypotheses")
...
print(f"Found {len(peer_hypotheses_target)} hypotheses about user_456")

# Get induction patterns
print("\nInduction patterns (via peer.get_inductions()):")
peer_inductions = peer.get_inductions(confidence="high")
print(f"Found {len(peer_inductions)} high-confidence patterns")

# Get behavioral patterns about another peer
print("\nBehavior patterns about another peer:")
peer_behavioral = peer.get_inductions(target="user_456", pattern_type="behavior")
print(f"Found {len(peer_behavioral)} behavior patterns")
