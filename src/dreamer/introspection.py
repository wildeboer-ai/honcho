...
def _introspection_max_tokens() -> int:
    return settings.DREAM.DEDUCTION_MODEL_CONFIG.max_output_tokens or 4096
...
async def run_introspection(
    ...
    try:
        llm_response = await honcho_llm_call(
            ...
            prompt=prompt,
            max_tokens=_introspection_max_tokens(),
            track_name="introspection",
            json_mode=True,
            temperature=0.3,
            trace_name="introspection",
        )
