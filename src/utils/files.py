    ...
    async def _extract_text_with_mistral_ocr(self, content: bytes) -> str:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                ocr_result = cast(object, response.json())
        ...
        if isinstance(ocr_result, Mapping):
            ocr_result_mapping = cast(Mapping[str, object], ocr_result)
            pages = ocr_result_mapping.get("pages")
        ...
        page_items = cast(list[object], pages)
        text_parts: list[str] = []
        for page in page_items:
            if isinstance(page, Mapping):
                page_mapping = cast(Mapping[str, object], page)
                markdown = page_mapping.get("markdown")
