...
async def list_inductions(
    ...
    pattern_type: str | None = Query(
        None,
        description="Filter by pattern type (preference/behavior/personality/tendency/temporal/conditional)",
    ),
...
async def get_induction(
    ...
    """
    induction = await crud.induction.get_induction(db, workspace_id, induction_id)
