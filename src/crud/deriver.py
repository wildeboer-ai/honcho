...
async def get_queue_status(
    ...
    *,
    ...
    units below DERIVER_REPRESENTATION_BATCH_MAX_TOKENS vs "ready" units that
    can be claimed. When DERIVER_FLUSH_ENABLED is true, nothing is stalled.
...
async def get_queue_work_units_query(
    ...
    *,
    ...
    """
    Build the Select statement for one row per unprocessed work unit.

    Same filter semantics as get_queue_status: observer/observed match the
    queue item payload, and session_name filters via the sessions table.
    """
    normalized_observer = observer if observer else None
    normalized_observed = observed if observed else None
    normalized_session_name = session_name if session_name else None
...
def classify_work_unit_row(row: Any) -> tuple[bool, int]:
    """Return (hit_threshold, tokens_until_threshold) for a work-units row."""
    ...
    if threshold_gated:
        pending_tokens = int(row.pending_tokens or 0)
        return pending_tokens >= batch_max_tokens, max(
            batch_max_tokens - pending_tokens, 0
        )
...
def _build_queue_work_units_query(
    ...
    *,
    ...
    """Build one row per unprocessed work_unit_key, aggregating items and tokens."""
    observer_name_expr = models.QueueItem.payload["observer"].astext
    observed_name_expr = models.QueueItem.payload["observed"].astext
