...
class PredictorAgent(BaseAgent):
    ...
    async def _retrieve_hypotheses(
        ...
        logger.info(
            f"Retrieved {len(filtered_hypotheses)} hypotheses for prediction generation"
        )
    ...
    async def _retrieve_existing_predictions(
        ...
        logger.info(
            f"Found {len(predictions)} existing predictions for hypothesis {hypothesis_id}"
        )
        return predictions
    ...
    async def _generate_predictions(
        ...
        premises_text = (
            "\n".join(
                [
                    f"- {p.content}" for p in source_premises[:10]
        ...
        existing_text = (
            "\n".join(
                [
                    f"- {p.content} (specificity: {getattr(p, 'specificity', 'N/A')})"
                    for p in existing_predictions[:10]
                ]
            )
            or "None"
        )
        ...
        try:
            ...
            logger.info(
                f"LLM completed with {len(response.tool_calls_made)} tool calls for hypothesis {hypothesis.id}"
            )
        ...
        logger.info(
            f"LLM generated {len(predictions_created)} prediction candidates for hypothesis {hypothesis.id}"
        )
    ...
    async def _store_predictions(
        ...
        for pred_data in predictions:
            try:
                ...
                if not is_novel:
                    logger.info(
                        f"Skipping duplicate prediction: {pred_data['content'][:50]}..."
                    )
                    continue
                ...
                logger.info(
                    f"Created prediction {prediction.id}: {prediction.content[:50]}..."
                )
