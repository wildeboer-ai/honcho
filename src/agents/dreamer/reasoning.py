...
═══════════════════════════════════════ 💭🧠"""
    ...
    try:
        ...
        if observation_count < min_observations_threshold:
            logger.info("Not enough observations for reasoning dream. Skipping.")
            metrics.finish()
            return metrics
        ...
        for hypothesis_id in hypothesis_ids[:max_iterations]:
            ...
            for prediction_id in prediction_ids:
                logger.info(f"🔬 Running Falsifier for prediction {prediction_id}...")
                await falsifier.execute(
                    {
                        "workspace_name": workspace_name,
                ...
                if prediction:
                    ...
                    logger.warning(
                        f"Prediction {prediction_id} not found after falsification"
                    )
...
async def _count_recent_observations(
    ...
    stmt = select(func.count(models.Document.id)).where(
...
async def _count_unfalsified_predictions(
    ...
    stmt = select(func.count(models.Prediction.id)).where(
        models.Prediction.workspace_name == workspace_name,
