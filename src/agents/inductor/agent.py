...
class InductorAgent(BaseAgent):
    ...
    async def _extract_patterns(
        ...
        for cluster in clusters[: self.config.max_inductions_per_run]:
            logger.info(
                f"Extracting pattern from cluster of {len(cluster)} predictions"
            )
            ...
            if (
                pattern_data
                and pattern_data.get("stability", 0)
                >= self.config.stability_score_threshold
            ):
                ...
                logger.info(
                    f"Created induction {induction.id}: {pattern_data['content']}"
                )
            else:
                stability = pattern_data.get("stability", 0) if pattern_data else 0
                logger.info(f"Skipped pattern with insufficient stability: {stability}")
    ...
    async def _generate_pattern(
        ...
        def tool_executor(tool_name: str, tool_input: dict[str, Any]) -> str:
            ...
            if tool_name == "create_induction":
                ...
                for pred in cluster:
                    ...
                    hyp = next(
                        (h for h in hypotheses if h.id == pred.hypothesis_id), None
                    )
        ...
        try:
            ...
            logger.info(
                f"Pattern extraction completed for cluster of {len(cluster)} predictions"
            )
