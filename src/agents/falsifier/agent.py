...
class FalsifierAgent(BaseAgent):
    ...
    async def _retrieve_predictions(
        ...
        if prediction_id:
            ...
            return (
                [prediction] if prediction and prediction.status == "untested" else []
            )
    ...
    async def _test_prediction(
        ...
        for iteration in range(self.config.max_search_iterations):
            logger.info(
                f"Falsification iteration {iteration + 1}/{self.config.max_search_iterations}"
            )

            # Prepare context for LLM
            previous_results = (
                "\n".join(search_results_history)
                if search_results_history
                else "None yet"
            )
        ...
        if (
            not contradicting_premise_ids
            and len(search_queries) >= self.config.max_search_iterations
        ):
    ...
    async def _execute_search_iteration(
        ...
        def tool_executor(tool_name: str, tool_input: dict[str, Any]) -> str:
            if tool_name == "generate_search_query":
                ...
                strategy = str(tool_input.get("strategy", ""))
                ...
                evidence_summary = str(tool_input.get("evidence_summary", ""))
                confidence = float(tool_input.get("confidence", 0.0))
                determination = str(tool_input.get("determination", "untested"))
                ...
                if (
                    ...
                    and confidence >= self.config.contradiction_confidence_threshold
                ):
                    state["status"] = "falsified"
                elif (
                    determination == "unfalsified"
                    and confidence >= self.config.unfalsified_confidence_threshold
                ):
                    state["status"] = "unfalsified"
                else:
                    state["status"] = "untested"

                return f"Evaluation recorded. Status: {state['status']}"

            elif tool_name == "stop_search":
                reason = str(tool_input.get("reason", "Search complete"))
                state["reasoning"] = f"Search stopped: {reason}"
        ...
        _response: HonchoLLMCallResponse[str] = await honcho_llm_call(
            model_config=settings.DIALECTIC.LEVELS["medium"].MODEL_CONFIG,
            prompt="",
            max_tokens=4000,
            messages=messages,
        ...
        contradictions = cast(list[str], state["contradictions"])
        search_summary = str(state.get("search_summary", ""))

        # Execute search if query was generated
        if isinstance(current_query, str) and current_query:
        ...
        return {
            "query": current_query if isinstance(current_query, str) else None,
            "contradictions": contradictions,
            "reasoning": str(state.get("reasoning", "")),
            "search_summary": search_summary,
            "status": str(state.get("status", "untested")),
        }
    ...
    async def _update_hypothesis_confidence(
        ...
        stmt = select(
            func.count()
            .filter(models.Prediction.status == "falsified")
            .label("falsified_count"),
            func.count()
            .filter(models.Prediction.status == "unfalsified")
            .label("unfalsified_count"),
