...
class AbducerAgent(BaseAgent):
    ...
    async def _generate_hypotheses(
        ...
        premises_text = "\n".join(
            [
                f"- [id:{p.id}] {p.content}" for p in premises[:50]
        ...
        existing_text = (
            "\n".join(
                [
                    f"- {h.content} (confidence: {h.confidence})"
                    for h in existing_hypotheses[:20]
                ]
            )
            or "None"
        )
        ...
        try:
            ...
            response: HonchoLLMCallResponse[str] = await honcho_llm_call(
                model_config=settings.DIALECTIC.LEVELS[
                    "medium"
                ].MODEL_CONFIG,  # Use medium reasoning level
            ...
            logger.info(
                f"LLM completed with {len(response.tool_calls_made)} tool calls"
            )
    ...
    async def _store_hypotheses(
        ...
        for hyp_data in hypotheses:
            try:
                ...
                logger.info(
                    f"Created hypothesis {hypothesis.id}: {hypothesis.content[:50]}..."
                )
