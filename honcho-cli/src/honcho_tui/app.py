...
def _queue_int(value: object) -> int:
    try:
...
class HonchoTUI(App[None]):
    ...
    TITLE: str | None = "HONCHO"
    CSS: ClassVar[str] = TCSS
        ...
        try:
            ...
            if not peer_id:
                try:
                    ...
                    if session_peers:
                        peer_id = str(session_peers[0].id)
                except Exception:
                    peer_id = ""
            ...
            if peer_id:
                ...
                try:
                    ...
                    conclusions = [
                        {"id": str(c.id), "content": str(c.content), "created_at": c.created_at}
    ...
    def _on_session_loaded(
        ...
        if peer_id:
            bar.peer_id = peer_id
        if queue:
            bar.pending = _queue_int(queue.get("pending", 0))
            bar.running = _queue_int(queue.get("running", 0))
    ...
    def _run_query(self, query: str) -> None:
        transcript = self.query_one("#transcript", TranscriptPanel)
        transcript.show_loading("querying dialectic…")
        self.run_worker(
