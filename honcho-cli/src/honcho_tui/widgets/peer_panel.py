...
def _card_to_str(card: object | None) -> str:
...
class PeerPanel(Widget):
    ...
    def load_peer(
        ...
        if queue is None:
            ...
            pending = _queue_int(queue.get("pending", 0))
            running = _queue_int(queue.get("running", 0))
            completed = _queue_int(queue.get("completed", 0))
